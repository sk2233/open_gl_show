...
uniform samplerCube Diffuse; // 预先生成的漫反射与高光 cube_map
uniform samplerCube Specular;
uniform vec3 LightPos;
uniform vec3 ViewPos;
uniform float Metallic; // 粗糙度
uniform float Roughness; // 金属度
...
void main() {
    ...
    Kd=vec3(1.0)-Ks;
    Kd*=1.0- Metallic;
    vec3 diffuseLight=texture(Diffuse,N).rgb;
    color+=Kd*diffuseLight*albedo; // 环境光漫反射
    vec3 specularLight =texture(Specular, R).rgb;
